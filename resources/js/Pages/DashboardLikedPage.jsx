...
export default function DashboardLikedPage({ navigateTo, auth, marketCurrencySymbol = 'POL' }) {
  ...
  const [pinnedProductsData, setPinnedProductsData] = useState([]);
  const [loadingPinnedProducts, setLoadingPinnedProducts] = useState(false);
  ...
  useEffect(() => {
    const fetchLikedProductsForDashboard = async () => {
      ...
      try {
        ...
        const normalizedProducts = (Array.isArray(likesData) ? likesData : [])
    ...
    const fetchPinnedProductsForDashboard = async () => {
      const userId = auth?.user?.idUser || auth?.user?.id;
      if (!userId) {
        setPinnedProductsData([]);
        return;
      }

      try {
        setLoadingPinnedProducts(true);
        ...
        const pinnedProducts = (Array.isArray(data) ? data : [])
          .map((pin) => pin?.produk || pin?.product || null)
          .filter((item) => item && (item.idproduk || item.id));

        if (pinnedProducts.length > 0) {
          setPinnedProductsData(pinnedProducts);
          return;
        }
        ...
        const allProducts = await productsRes.json();
        setPinnedProductsData(
          (Array.isArray(allProducts) ? allProducts : []).filter((product) =>
            pinnedIds.includes(String(product?.idproduk || product?.id))
          )
        );
      } catch (error) {
        console.error('Error fetching pinned products for dashboard:', error);
        setPinnedProductsData([]);
      } finally {
        setLoadingPinnedProducts(false);
      }
    };

    fetchLikedProductsForDashboard();
    fetchPinnedProductsForDashboard();
